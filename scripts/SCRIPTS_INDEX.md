...
> **自动化脚本根目录** - 系统级的自动化健康监测、配置验证、部署脚本
...
脚本系统配置定义在 `scripts_config.json`，包含：

- **executionRules**: 执行规则（调度、权限、错误处理）
- **monitoring**: 监控配置（指标、告警、报告）
- **maintenance**: 维护规则（清理、备份、更新）
- **integration**: 集成配置（与项目、CI的集成）
- **rulesCompliance**: 规则合规性验证

## 相关文档

- 脚本配置: `scripts_config.json`
- 脚本说明: `scripts_README.md`
- 项目配置: `project_config.json`
- 规则系统: `rules/`
- [自动化脚本说明](../../docs/project_docs/05-automation-scripts.md)
