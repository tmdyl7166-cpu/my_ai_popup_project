...
- **meta**: 元数据和依赖关系定义
- **architecture**: 架构描述
  - **style**: 架构风格（分层架构+插件模式）
  - **layers**: 各层定义（Presentation、Application、AI Processing等）
- **modules**: 模块详细定义
  - 每个模块包含：描述、路径、组件、职责、输入输出、依赖关系
  - 模块包括：frontend、backend、ai、processing、integrations、utils、config
- **relationships**: 模块间关系
  - **dataFlow**: 数据流路径定义
  - **dependencies**: 依赖关系图
  - **communication**: 通信模式（同步请求-响应、异步事件驱动等）
