...
class AppConfig:
    ...
    def __post_init__(self):
        ...
        # 使用路径解析器设置相对路径
        self.project_root = str(resolver.get_project_root())
        self.assets_dir = str(resolver.get_assets_dir())
        self.models_dir = str(resolver.get_models_dir())
        self.output_dir = str(resolver.get_output_dir())
        # 设置日志文件路径（相对于项目根目录）
        self.log_file = str(resolver.get_logs_dir() / "app.log")
    ...
    def load(cls, config_path: Optional[str] = None) -> "AppConfig":
        ...
        if config_path:
            ...
            # 尝试加载项目根目录的.env文件
            resolver = PathResolver()
            env_path = resolver.get_project_root() / ".env"
            if env_path.exists():
        ...
        return cls(
            ...
            gpu_memory_fraction=float(os.getenv(
            ...
            processing_quality=os.getenv(
                "PROCESSING_QUALITY", cls.processing_quality),
            virtual_camera_enabled=os.getenv(
                "VIRTUAL_CAMERA_ENABLED",
                str(cls.virtual_camera_enabled)).lower() == "true",
            virtual_camera_device=os.getenv(
                "VIRTUAL_CAMERA_DEVICE", cls.virtual_camera_device),
            virtual_camera_width=int(os.getenv(
                "VIRTUAL_CAMERA_WIDTH", cls.virtual_camera_width)),
            virtual_camera_height=int(os.getenv(
                "VIRTUAL_CAMERA_HEIGHT", cls.virtual_camera_height)),
            virtual_camera_fps=int(os.getenv(
                "VIRTUAL_CAMERA_FPS", cls.virtual_camera_fps)),
